...
The results show that the neural network is by far the fastest method (around 1.5 times faster than FFT in this specific case), at the cost of a higher error and larger error variance (median error = 1.63%).
Monte Carlo, while significantly slower, produces prices that are very close to the Carr–Madan benchmark and exhibits greater stability in its error distribution (median error = 0.24%; error std = 0.05).
